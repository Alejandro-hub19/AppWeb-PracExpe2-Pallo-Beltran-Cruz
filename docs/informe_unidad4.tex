...
\begin{enumerate}
  ...
  \item \textbf{\texttt{500} en vez de \texttt{400} ante un parámetro inválido (severidad
    ...
    \texttt{500 Internal Server Error}, cuando la causa es una entrada mal formada del
    cliente y debería responder \texttt{400}, según \textcite{masse2011}
    (Sección~\ref{sec:biblio}).
  \item \textbf{CRUD de Categorías sin interfaz (severidad media, funcionalidad incompleta).}
    ...
    crear, editar o dar de baja una categoría sin usar la API directamente.
  ...
  \item \textbf{Alta de Entrenador incompleta por diseño (severidad alta, funcionalidad
    ...
    aislar la causa raíz y confirmar la solución, no solo reportar el síntoma.
\end{enumerate}
